...
**"is not authorized to perform: bedrock:InvokeModel"** — Ensure
`enable_bedrock = true` (the default) and run `terraform apply`.
...
Then restart the instance or the openclaw service to pick up the new
keys.
